...
async def get_collections(
    ...
    collections = await crud.get_collections(session, from_date, to_date, interval, desc, skip, take)
    result = [FromDB.to_collection(collection, False, False) for collection in collections]
    return result


@router.post("/")
async def create_collection(collection: Annotated[CollectionCreate9, Body()], session: AsyncSession = Depends(db.get_session)) -> CollectionOut:
...
async def delete_collection(collection_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> None:
    await crud.delete_collection(session, collection_id)


@router.get("/{collection_id}")
async def get_collection(collection_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> CollectionOut:
...
async def get_alliances_from_collection(collection_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> list[AllianceOut]:
...
async def get_alliance_from_collection(collection_id: Annotated[int, Path(ge=0)], alliance_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> AllianceOut:
    if not (await crud.has_collection(session, collection_id)):
        raise HTTPException(404, f"A collection with the ID `{collection_id}` was not found.")

    alliance = await crud.get_alliance_from_collection(session, collection_id, alliance_id, True)
    if not alliance:
...
async def get_top_100_from_collection(
    ...
    users = await crud.get_top_100_from_collection(session, collection_id, skip, take)
    result = [FromDB.to_user(user) for user in users]
    return result


@router.get("/{collection_id}/users")
async def get_users_from_collection(collection_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> list[UserOut]:
    if not (await crud.has_collection(session, collection_id)):
        raise HTTPException(404, f"A collection with the ID `{collection_id}` was not found.")
...
async def get_user_from_collection(collection_id: Annotated[int, Path(ge=0)], user_id: Annotated[int, Path(ge=0)], session: AsyncSession = Depends(db.get_session)) -> UserOut:
    if not (await crud.has_collection(session, collection_id)):
        raise HTTPException(404, f"A collection with the ID `{collection_id}` was not found.")

    user = await crud.get_user_from_collection(session, collection_id, user_id, True)
    if not user:
