...
async def test_save_collection(include_alliances: bool, include_users: bool, expected_exception: AbstractContextManager, session: AsyncSession, new_collection: CollectionDB):
...
async def test_save_collection_non_unique(override_id: int, override_timestamp: datetime, expected_exception: AbstractContextManager, session: AsyncSession, new_collection: CollectionDB):
    with expected_exception:
