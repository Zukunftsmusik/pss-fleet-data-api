...
def test_get_users_from_collection_invalid_id(collection_id: int, assert_error_code, client: TestClient):
    with client:
...
def test_get_users_from_collection_non_existing_id(assert_error_code, client: TestClient):
    with client:
